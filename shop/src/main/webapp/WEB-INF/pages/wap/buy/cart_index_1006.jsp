       ...
       <div class="item">>
        ...
        <div class="info">
         ...
         <div class="bottom">
          参与期数：
          <div class="number">
           <input class="qishu-input" type="text" data-id="${item.product.id}" data-max="${item.product.leaveNum}" value="${item.qiNum}" style="color: #DA3752;" /> <a
            class="num-btn btn-qishu-plus" data-pro="qi_plus" href="javascript:void(0);">+</a> <a class="num-btn btn-qishu-minus" data-pro="qi_minus" href="javascript:void(0);">-</a>
          </div>
          期
         </div>
 ...
 <script type="text/javascript">
		...
		$(function() {
			...
			$(".btn-qishu-plus").click(function() {
				...
				var qty = $(this).parent().children('input.qishu-input').val() * 1;
				if (qty < max) {
					$(this).parent().children('input.qishu-input').val(qty + 1);
				}
				update_qishu_cart(id, $(this).parent().children('input.qishu-input').val(), '');
			});
			...
			$("input.qishu-input").blur(function() {
				var id = $(this).attr('data-id');
				var max = $(this).attr('data-max') * 1;
				if ($(this).val() > max) {
					$(this).val(max);
			...
			function updatecart(id, qty, multi) {
				...
				$.post('/cart/updateCartNum.json', {
					id : id,
					buyNum : qty,
					qiNum : $(".btn-qishu-plus").parent().children('input.qishu-input').val(),
				}, function(data) {
					//$('#total').html(data.total);
					show_total();
				}, 'json');
			}
			function update_qishu_cart(id, qty, multi) {
				var ids = '';
				var i = 0;
				$("input[name='ids']").each(function() {
					if ($(this).is(':checked')) {
				...
				$.post('/cart/updateCartNum.json', {
					id : id,
					buyNum : $(".btn-plus").parent().children('input.num-input').val(),
					qiNum : qty,
				}, function(data) {
					//$('#total').html(data.total);
					show_total();
						...
						for (var i = 0; i < data.message.length; i++) {
							...
							for ( var j in arr) {
								if (arr[j] == o.product.id) {
									total += o.product.singlePrice * o.buyNum * o.qiNum;
								}
							}
						}
						$('#total').html(total);
