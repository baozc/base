...
<body>
 ...
 <c:choose>
  ...
  <c:otherwise>
   <form action="/member/buy/buycheck" method="POST" id="buyForm" onsubmit="return validateForm()"  style="display:none">
    <div id="content" class="container main">
     <div class="cart-list">
      ...
      <c:forEach items="${results}" var="item">
       ...
       <div class="item">>
        <div class="pic">
         <a href="/product/${item.id}.htm"><img alt="${item.scoreProduct.name}" src="${item.scoreProduct.logoPath}" width="64" /></a>
        </div>
        <div class="info">
         <div class="title to">
          <%--          <c:if test="${item.success==false }">disabled</c:if> --%>
          <input value="${item.scoreProduct.id}" style="margin-right: 0.6rem; margin-left: 0.4rem;" name="ids" type="checkbox"
           <c:if test="${item.success==false }">disabled</c:if> <c:if test="${item.success==true }">checked</c:if> /> <a href="/product/${item.scoreProduct.id}.htm">${item.scoreProduct.name}</a>
         </div>
         <p>
          价格： <em>￥${item.scoreProduct.singlePrice}</em> 元
         </p>
         <div class="bottom">
          购买数量：
          <div class="number">
		   <c:if test="${item.success==true }">
            <c:set var="totalPrice" value="${totalPrice+item.buyNum*item.scoreProduct.singlePrice}" />
           </c:if>
           <input class="num-input" type="text" data-id="${item.id}"  value="${item.buyNum}" style="color: #DA3752;" /> <a
         ...
         <a class="del" href="javascript:;" onclick="delBuy('${item.id}',1)"><i class="ap-icon icon-del"></i></a>
        </div>
       </div>
      </c:forEach>
     </div>
				...
				$.post('/buy/total.json', {
					...
					if (data.isSuccess) {
						...
						for (var i = 0; i < data.message.length; i++) {
							...
							//如果选中
							for ( var j in arr) {
								if (arr[j] == o.scoreProduct.id) {
									total += o.scoreProduct.singlePrice * o.buyNum;
								}
							}
						}
						$('#total').html(total);
