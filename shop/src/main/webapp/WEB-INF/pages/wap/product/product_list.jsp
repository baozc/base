...
<div class="sort">
    <ul class="ui-clr">
        <li id="li_cate" class="DESC">
            <c:set var="cateName" value="全部商品"/>
            <c:forEach items="${pcates}" var="pcate">
                <c:if test="${cateId eq pcate.id }">
                    <c:set var="cateName" value="${pcate.name }"/>
                    <c:set var="hasCateName" value="1"/>
                </c:if>
            </c:forEach>
            <a>${cateName }<i class="ap-icon icon-sort"><em></em></i></a></li>
        <li class="li_sort<c:if test="${orderType==1 }"> on </c:if>"><a href="/product/list/${cateId}-0-1-1.html">人气</a>
        </li>
        <li class="li_sort<c:if test="${orderType==3 }"> on </c:if>"><a href="/product/list/${cateId}-0-3-1.html">最新</a>
        </li>
        <li class="li_sort<c:if test="${orderType==2 }"> on </c:if>"><a
                href="/product/list/${cateId}-0-2-1.html">剩余人次</a></li>
        <c:choose>
            <c:when test="${orderType==4}">
                <li class="li_sort on ASC"><a href="/product/list/${cateId}-0-5-1.html"> 总需人次 <i
                        class="ap-icon icon-sort"><em></em></i>
                </a></li>
            </c:when>
            <c:when test="${orderType==5}">
                <li class="li_sort on DESC"><a href="/product/list/${cateId}-0-4-1.html"> 总需人次 <i
                        class="ap-icon icon-sort"><em></em></i>
                </a></li>
            </c:when>
            <c:otherwise>
                <li class="li_sort"><a href="/product/list/${cateId}-0-4-1.html"> 总需人次 <i
                        class="ap-icon icon-sort"><em></em></i>
                </a></li>
            </c:otherwise>
        </c:choose>
...
<div class="pro-view">
    ...
    <div class="cate">
        <ul class="ui-clr">
            <li>
                <a href="/product"
                   <c:if test="${empty hasCateName }">class="on"</c:if> ><i class="ap-icon d0"></i>全部趣购</a></li>
            <c:forEach items="${pcates}" var="pcate">
                <li>
                    <a href="/product/list/${pcate.id }"
                       <c:if test="${cateId eq pcate.id }">class="on"</c:if>>
                        <i class="ap-icon d2"></i>${pcate.name }
                    </a>
                </li>
            </c:forEach>
        </ul>
    ...
    <ul class="paiList list">
        <c:forEach items="${page }" var="item">
            <c:if test="${item.deletestatus eq false }">
                <li class="ui-clr">
                    <div class="pic">
                        <a href="${base }/product/${item.id}" title="${item.name}">
                            <img src="${item.logopath }"  alt="${item.name }" id="buy_img_${item.id }"/>
                        </a>
                    </div>
                    <div class="info">
                        <p class="title">
                        ...
                        <div class="progressBar">
                            <p class="wrap-bar">
                                <span class="bar"
                                      style="width: <fmt:formatNumber value="${item.usednum/item.totalnum*100}"
                                                                      pattern="0.0#"/>%">"></span>
                            </p>
                            <div class="txt ui-clr">
                                <span class="ui-left">${item.usednum }人已参与</span> <span
                                    class="ui-right">剩余${item.leavenum }</span>
                            </div>
                        </div>
                    </div>
                    <button class="add-btn" onclick="javascript:window.location.href='${base }/product/${item.id}'">
