  ...
  <div class="pro-view shareList">
   <ul class="goodList" id="win-list">
    <li class="item ui-clr item-db"></li>
	 <c:forEach items="${YgProduct }" var="item">
	<li class="item ui-clr item-db">
					<div class="pic" style="overflow: hidden; height: 100px; font-size: 1.3rem">
					 <a href="/product/${item.id }.html"><img src="${item.logoPath }" style="height: 100px; width: 110px; position: absolute;"></a>
					<p class="timeago"><span>${item.winUserIpAddr}</span></p>
					</div>
					<div class="info">
					<p class="title" style="border-bottom: 0; padding-bottom: 0"><a href="/product/${item.id}.html">(第${item.period}期)${item.name}
					</a></p>
					<p class="itemP">参与人数：<span class="">${item.usedNum}</span></p>
					<p class="" style="font-weight: blod;">幸运号码：<span class="color">${item.winNo}</span></p>
					<p class="itemP"><span>幸运用户</span>：<a href="/win" class="color">${item.winUserNickName}</a><a href="/product/${item.id}/new.php" class="buynext">参与下期</a></p>
					<div></div>
					</div>
					</li>
		  </c:forEach>
