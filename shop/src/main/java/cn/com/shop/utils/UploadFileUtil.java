...
import cn.com.shop.mapper.YygAccessoryMapper;
...
public class UploadFileUtil {
	...
	/** 文件表dao */
	private static YygAccessoryMapper accessoryDao = SpringContextHolder.getBean(YygAccessoryMapper.class);
	/** * 文件根路径 */
	public static String rootPath = SpringContextHolder.getBean(UploadFileUtilHelper.class).getRootPathTemp();
	 ...
	 */
	public static YygAccessory uploadFile(MultipartFile file, String storageFilePath, String storageFileName, Long userId) throws BusinessException {
	...
	public static void deleteFile(long accessoryId) {
		YygAccessory accessory = accessoryDao.selectByPrimaryKey(accessoryId);
		// 在硬盘删除原文件
		if (accessory != null) {
		...
		if (accessory != null) {
			...
			try {
				...
				if (accessory.getId() != null) {
					accessoryDao.deleteByPrimaryKey(accessoryId);
				}
			} catch (Throwable e) {
				logger.error(e.getMessage(), e);
			}
	 ...
	 * @param accessory
	 */
	public static void deleteFileAndAccessory(YygAccessory accessory) {
		...
		if (accessory != null) {
			try {
				...
				// 删除数据库记录
				if (accessory.getId() != null) {
					accessoryDao.deleteByPrimaryKey(accessory.getId());
				}
			} catch (Throwable e) {
				logger.error(e.getMessage(), e);
			}
	 ...
	 * @param accessory
	 */
	public static void deleteFile(YygAccessory accessory) {
	 ...
	 * @throws BusinessException
	 */
	public static YygAccessory uploadFile(FileUploadVO fileUploadVO) throws BusinessException {
		...
		try {
			...
			// 返回保存数据库对象
            YygAccessory accessory = new YygAccessory();
			accessory.setPath(filePath);
			accessory.setName(fileName);
			accessory.setSize(Float.valueOf(data.length+""));
