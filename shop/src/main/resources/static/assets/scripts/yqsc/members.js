		...
		$(document).on("click", ".pf-table-btn-delete", function() {
			...
			showConfirmDialog(function() {
				...
				$.ajax({
					...
					cache : false,
					dataType : "json",
					url : "/yqsc/members/delete.json?id=" + rowData.id,
		...
		$("#pf-table-members").bootstrapTable({
			...
			columns : [ {
				...
				formatter : function(value, row, index) {
					return (value == 1) ? "管理员" : (value == 2)?"虚拟会员": "会员";
				}
			}, {
				field : 'innerUser',
				title : '内部用户',
				...
				sortable : false,
				formatter : function(value, row, index) {
					var html = ' <a _href="/yqsc/userAccountDetail?userId=' + row.id + '" class="am-btn am-btn-success am-btn-xs newAccountAmount">充值</a>&nbsp;'
					html = html + '<a href="/yqsc/members/edit?id=' + row.id + '" class="am-btn am-btn-success am-btn-xs">编辑</a>&nbsp;'
