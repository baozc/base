					...
					columns : [
							...
							},
							{
								...
								formatter : function(value, row, index) {
									...
									if(row.status==2){
										html+='<a onclick="shouhuo('+row.id+')" class="am-btn hl-btn-green  am-btn-xs style="margin-right:5px;">强制收货</a>';
									}
									return html;
								}
...
function shouhuo(orderid){
	if(confirm("确定要强制收货吗？")){
		$.ajax({
			...
			url : "/yqsc/ygOrder/finish_order.json",
			success : function(data) {
				if (data.isSuccess) {
					showPopup("操作成功!");
					$("#pf-table-members").bootstrapTable("refresh");
