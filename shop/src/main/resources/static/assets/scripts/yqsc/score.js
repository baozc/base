		...
		$(document).on("click", ".pf-table-btn-delete", function() {
			...
			showConfirmDialog(function() {
				...
				$.ajax({
					...
					cache : false,
					dataType : "json",
					url : "/yqsc/score/delete.json?id=" + rowData.id,
			...
			}, "是否删除商户：" + rowData.createBy+"提现"+rowData.score/100+"申请?", "系统");
		});

		// 审核按钮
		$(document).on("click", ".am-btn-success", function() {
			var $tr = $(this).closest("tr");
			// 行数据
			var rowData = $tr.data("data");
			showConfirmDialog(function() {
				...
				$.ajax({
					...
					cache : false,
					dataType : "json",
					url : "/yqsc/score/audit.json?done=true&id=" + rowData.id,
					success : function(data) {
						if (data != undefined && data.isSuccess == true) {
							showPopup("审核成功!");
							$("#pf-table-members").bootstrapTable("refresh");
			...
			}, "是否审核商户" + rowData.createBy+"提现"+rowData.score/100+"?", "系统");
		});
		...
		$(document).on("click", ".btn-primary", function() {
			...
			showConfirmDialog(function() {
				...
				$.ajax({
					...
					cache : false,
					dataType : "json",
					url : "/yqsc/score/audit.json?done=false&id=" + rowData.id,
					success : function(data) {
						if (data != undefined && data.isSuccess == true) {
							showPopup("拒绝成功!");
							$("#pf-table-members").bootstrapTable("refresh");
			...
			}, "是否拒绝审核商户" + rowData.createBy+"提现"+rowData.score/100+"?", "系统");
		});
	},
	// 初始化应用表格
	_init_app_table : function() {
		$("#pf-table-members").bootstrapTable({
			...
			columns : [ {
				...
				formatter : function(value, row, index) {
					if (row.audit==0) {
						var html = ' <a  class="am-btn am-btn-success am-btn-xs ">通过</a>&nbsp;'
