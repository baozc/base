...
function getVoiceVerify(btn,mobile){
    ...
    var scode = $('#scode').val();
    var D = {mobile:mobile,submit:'ok',scode:scode };
...
function RemainTime2(btn){
    ...
    if (iTime >= 0){
        ...
        if(iTime==0){
            ...
            Account = setTimeout(function(){ RemainTime2(btn); },1000);
            iTime=iTime-1;
        }
    }
    $(btn).val(sTime);
...
function addToCart(id,type,obj,pre){
    ...
    var multi = ($('#multi_'+id).length>0) ? $('#multi_'+id).val() : 1;
    $.post('/yunbuy/addtocart',{id:id,qty:qty,multi:multi},function(data){
        if(data.error){
            var params = ' ';
            if(data.url){
                params = function(){
...
//揭晓商品轮询
$.getScript('/assets/style/arttemplate/template-native.js');
...
function compaceIds(skin){
    ...
    $.ajax({
        type: 'post',
        url: apiUrl+'/m/new.php?clause=ajaxIds',
                ...
                if(list.length>0){
                    for(var i = 0; i < list.length; i++){
                        if(list[i].wait_status==0){
							//不存在，才加入
							if($('#itemDjx' + list[i].buy_id).length <= 0){
								$('#itemDjx' + list[i].buy_id).remove();
								
								window.clearInterval(timeID_jx[list[i].buy_id]);
								window.clearInterval(timeID_h_jx[list[i].buy_id]);
