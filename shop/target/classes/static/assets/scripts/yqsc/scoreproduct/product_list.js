...
var ProductObject = {
	...
	_reg_search_btn_event : function() {
		$("#pf-btn-search").click(function() {
			$("#pf-table-product").bootstrapTable("refresh", {
				url : "/yqsc/scoreproduct/search.json?cateId=" + $("#cateId").val() + "&searchText=" + encodeURIComponent($("#searchbox").val()),
				pageNumber : 1
			});
		});
	},
		...
		$(document).on("click", ".pf-btn-delete-product", function() {
			...
			showConfirmDialog(function() {
				$.ajax({
					...
					dataType : "json",
					url : "/yqsc/scoreproduct/delete.json",
	...
	_init_app_table : function() {
		$("#pf-table-product").bootstrapTable(
				{
					method : 'get',
					url : "/yqsc/scoreproduct/search.json",
					cache : false,
					// height : 400,
					...
					columns : [
							...
							},
							{
								...
								formatter : function(value, row, index) {
									var html='&nbsp;'
										+ '<a href="/yqsc/scoreproduct/edit/' + row.id
