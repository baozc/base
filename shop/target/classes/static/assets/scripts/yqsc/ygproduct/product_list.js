...
var ProductObject = {
	...
	_reg_search_btn_event : function() {
		$("#pf-btn-search").click(function() {
			$("#pf-table-product").bootstrapTable("refresh", {
				url : "/yqsc/ygproduct/search.json?cateId=" + $("#cateId").val() + "&searchText=" + encodeURIComponent($("#searchbox").val()),
				pageNumber : 1
			});
		});
	},
		...
		$(document).on("click", ".pf-btn-delete-product", function() {
			...
			showConfirmDialog(function() {
				$.ajax({
					...
					dataType : "json",
					url : "/yqsc/ygproduct/delete.json",
	...
	_init_app_table : function() {
		$("#pf-table-product").bootstrapTable(
				{
					method : 'get',
					url : "/yqsc/ygproduct/search.json",
					cache : false,
					// height : 400,
					...
					columns : [
							...
							},
							{
								...
								formatter : function(value, row, index) {
									...
									html+='&nbsp;<a href="/yqsc/ygproduct/buyRecord/' + row.id
									+ '.html" class="btn btn-primary btn-xs">购买记录</a>';
									//已结束
									if(row.status>1){
										return html;
									}
									html+='&nbsp;<a href="/yqsc/ygproduct/edit/' + row.id
