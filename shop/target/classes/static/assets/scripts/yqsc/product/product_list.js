	...
	_reg_search_btn_event : function() {
		$("#pf-btn-search").click(function() {
			$("#pf-table-product").bootstrapTable("refresh", {
				url : "/yqsc/product/search.json?cateId=" + $("#cateId").val() + "&searchText=" + encodeURIComponent($("#searchbox").val()),
				pageNumber : 1
			});
		});
	},
		...
		$("#pf-table-product").bootstrapTable(
				{
					...
					columns : [
							...
							},
							{
								...
								formatter : function(value, row, index) {
									var html='&nbsp;'
										+ '<a href="/yqsc/product/edit/' + row.id
